...
def enter_details():
    if request.method == 'POST':
        ...
        predictor = AutopliusPredictorByMake(make)
        ml_model, score = predictor.train_model()
        #params_trans = predictor.polynomial_features.fit_transform(params)
        ...
        return render_template('valuation.html', result=f'{round(int(valuation), 0):,}',
            ...
            # return render_template('main_page.html', result='Neteisingai įvesti arba praleisti '
            #                                                  'duomenys')
